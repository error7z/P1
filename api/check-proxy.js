...
export default async function handler(req, res) {
    ...
    try {
        // Deep Intel Fetch
        const ipInfo = await axios.get('http://ip-api.com/json?fields=query,status,country,city,isp,as,reverse', { 
            httpAgent: agent, 
            httpsAgent: agent,
            timeout: 8000 
        });
        ...
        res.status(200).json({
            ...
            googleScore: isExtremelyBad ? "POOR (Spam Risk)" : (isBad ? "NEUTRAL" : "EXCELLENT (Inbox Ready)")
        });

    } catch (error) {
        res.status(500).json({ success: false, error: "Proxy Tunnel Failed. Check credentials or Port 25/465 blocking." });
    }
}
